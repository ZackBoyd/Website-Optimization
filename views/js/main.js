...
var resizePizzas = function(size) {
  ...
  //Pull all the random pizza containers into one list
    var randomPizzaContainerElements = document.getElementsByClassName("randomPizzaContainer");
  //Measure constants from random pizza container elements to use in loop
  	var numPizzas = randomPizzaContainerElements.length;
...
//Contants number of background pizzas to loop through, the length of that array and the most recent scroll level
...
//Loop to fill phaseArray with 5 unique values
  ...
  for (var i = 0; i < numItems; i++) {
    ...
    var offsetAmount = ((items[i].basicLeft + 100 * phase)-1024) + 'px';
    items[i].style.transform = 'translateX(' + offsetAmount + ')';
  }
...
//Figure out how many pizzas I can fit on the screen
  var s = 256;
  var pizzaRows = Math.ceil(window.innerHeight / s);
  var pizzaColumns = Math.ceil(window.innerWidth / s);
  var numPizzaSquares = pizzaRows * pizzaColumns;

  for (var i = 0; i < numPizzaSquares; i++) {
    var elem = document.createElement('img');
    elem.className = 'mover';
    elem.src = "images/pizza.png";
    elem.style.height = "100px";
    elem.style.width = "73.333px";
    elem.basicLeft = (i % pizzaColumns) * s;
    elem.style.top = (Math.floor(i / pizzaColumns) * s) + 'px';
